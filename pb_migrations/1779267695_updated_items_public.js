...
migrate((app) => {
  ...
  // remove field
  collection.fields.removeById("_clone_0mCw")

  // remove field
  collection.fields.removeById("_clone_MNiX")

  // remove field
  collection.fields.removeById("_clone_bdG3")

  // remove field
  collection.fields.removeById("_clone_4RNq")
  ...
  // update collection data
  unmarshal({
    "viewQuery": "SELECT \n  items.id, items.name, items.image, items.externalImgUrl, \n  users.username\nFROM items\nLEFT JOIN users on items.owner = users.id"
  }, collection)
